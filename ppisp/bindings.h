...
void ppisp_forward(
    ...
    // Dimensions
    int num_pixels, int num_cameras, int num_frames, int resolution_w,
    int resolution_h, int camera_idx, int frame_idx);
...
void ppisp_backward(
    // Parameters (per-camera/per-frame)
    const float *exposure_params,
    const float *vignetting_params, const float *color_params,
    ...
    float *v_exposure_params,
    float *v_vignetting_params, float *v_color_params, float *v_crf_params,
    float *v_rgb_in,
    // Dimensions
    int num_pixels, int num_cameras, int num_frames, int resolution_w,
    int resolution_h, int camera_idx, int frame_idx);
...
torch::Tensor ppisp_forward_tensor(
  ...
  ppisp_forward(
      exposure_params.data_ptr<float>(),
      vignetting_params.data_ptr<float>(), color_params.data_ptr<float>(),
      crf_params.data_ptr<float>(), rgb_in.data_ptr<float>(),
      rgb_out.data_ptr<float>(), pixel_coords.data_ptr<float>(), num_pixels,
      num_cameras, num_frames, resolution_w, resolution_h, camera_idx,
      frame_idx);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
           torch::Tensor>
ppisp_backward_tensor(torch::Tensor exposure_params,
                      torch::Tensor vignetting_params,
                      torch::Tensor color_params, torch::Tensor crf_params,
                      torch::Tensor rgb_in, torch::Tensor rgb_out,
                      torch::Tensor pixel_coords, torch::Tensor v_rgb_out,
  ...
  ppisp_backward(
      ...
      vignetting_params.data_ptr<float>(), color_params.data_ptr<float>(),
      crf_params.data_ptr<float>(), rgb_in.data_ptr<float>(),
      rgb_out.data_ptr<float>(), pixel_coords.data_ptr<float>(),
      v_rgb_out.data_ptr<float>(),
      v_exposure_params.data_ptr<float>(),
  ...
  return std::make_tuple(v_exposure_params,
                         v_vignetting_params, v_color_params, v_crf_params,
                         v_rgb_in);
}
