...
__global__ void ppisp_bwd_kernel(
    int batch_size, int num_cameras, int num_frames,
    const float *__restrict__ exposure_params,
    const VignettingChannelParams *__restrict__ vignetting_params,
    const ColorPPISPParams *__restrict__ color_params,
    ...
    int frame_idx)
{
    ...
    if (tid < batch_size)
    {
        ...
        if (frame_idx != -1)
        {
            apply_exposure_bwd(rgb_input, exposure_params[frame_idx], grad_rgb, grad_rgb, grad_exposure_local);
        }
...
void ppisp_forward(const float *exposure_params,
                   const float *vignetting_params, const float *color_params,
                   const float *crf_params, const float *rgb_in, float *rgb_out,
                   const float *pixel_coords, int num_pixels, int num_cameras, int num_frames,
                   int resolution_w, int resolution_h, int camera_idx, int frame_idx)
{
    ...
    ppisp_kernel<<<blocks, threads>>>(
        num_pixels, num_cameras, num_frames,
        exposure_params,
        ...
        reinterpret_cast<const float2 *>(pixel_coords), resolution_w, resolution_h,
...
void ppisp_backward(const float *exposure_params,
                    const float *vignetting_params, const float *color_params,
                    const float *crf_params, const float *rgb_in, const float *rgb_out,
                    ...
                    int frame_idx)
{
    ...
    ppisp_bwd_kernel<PPISP_BLOCK_SIZE><<<blocks, threads>>>(
        ...
        reinterpret_cast<const float3 *>(v_rgb_out),
        v_exposure_params,
        ...
        reinterpret_cast<float3 *>(v_rgb_in),
        reinterpret_cast<const float2 *>(pixel_coords),
